...
for i in range(1, 13):
    ...
    try:
        ...
        if '.navbar .nav-actions {' in content and 'position: fixed !important;' in content:
            print(f"⚠ {filename} већ има mobile navbar fix")
            continue
        
        # Find the last @media (min-width: 1282px) block and add our fix after it
        pattern = r'(@media \(min-width: 1282px\) \{[^\}]*\})\s*\n\s*(</style>)'
        ...
        if re.search(pattern, content, re.DOTALL):
            ...
            print(f"⚠ Нисам пронашао одговарајући патерн у {filename}")
